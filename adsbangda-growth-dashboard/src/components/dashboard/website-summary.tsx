  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <MiniStat label="Pengunjung" value={formatNumber(latest.visitors ?? 0)} deltaPct={pctDelta(latest.visitors, previous?.visitors)} />
        <MiniStat label="Leads (Form)" value={formatNumber(latest.conversions ?? 0)} deltaPct={pctDelta(latest.conversions, previous?.conversions)} />
        <MiniStat
          label="Conversion Rate"
          value={latest.visitors ? `${((latest.conversions ?? 0) / latest.visitors * 100).toFixed(2)}%` : "—"}
          deltaPct={null}
        />
        <MiniStat label="Bounce Rate" value={latest.bounceRate != null ? `${latest.bounceRate}%` : "—"} deltaPct={pctDelta(latest.bounceRate, previous?.bounceRate)} deltaGoodDirection="down" />
      </div>
